...
contract StakingPoolProxy is StakingPoolStorageStructure {
    ...
    */
    ...
    ) public initializer onlyPoolCreator
    {
        ...
        if ( _addrs[2] == address(0)) 
            oracleDecimals = _variables[11];
        else
            oracleDecimals = PriceConsumerUpgradeable.getDecimals();
        ...
        for (l = 0; l < _ranks.length; l++) {
            ...
            if(_ranks[l+1] != 0) {
                ...
                rewardRates = rewardRates.add((_ranks[l+1].sub(_ranks[l])).mul(_percentages[l+1]));
            }
            ...
            prizeRewardRates.push(
                PrizeRewardRate({
                    rank: _ranks[l], 
                    percentage: _percentages[l]
                })
            );
        }

        require(_ranks[l-1] <= 25, "last rank must be less than or equal to 25");
